...
usersRouter.post("/register", async (req, res) => {
  ...
  try {
    ...
    if (error.code === 11000) {
      res
        .status(403)
        .send({ message: "Account with that email already exists" });
    } else {
      console.log(error);
      res
        .status(500)
        .send({ message: "Server error while registering account" });
    }
  }
});
...
usersRouter.put("/edituser/:id", async (req, res) => {
  ...
  try {
    if (
      updatedData.name.length < 4 ||
      updatedData.email.length < 4 ||
      updatedData.password.length < 4
    ) {
